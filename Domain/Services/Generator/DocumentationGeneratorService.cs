    ...
    public class DocumentationGeneratorService
    {
        ...
        public void ParseFromEntry(StringBuilder builder, GeneratorModel model, EntryModel entry)
        {
            ...
            try
            {
                ...
                entityList.AppendLine($"|**Name**|**Type**|**Required**|**Key**|**Auto**|**Parent**|**Description**|");
        ...
        private void BuildProperties(StringBuilder result, int tab, EntryModel entry)
        {
            foreach (MapperProperty p in entry.Properties)
            {
                ...
                var notNull = p.IsRequired ? CheckYes : CheckNo;

                //result.AppendCode(tab, $"+{p.TypeDB} {p.NameDB}", 1);
                //result.AppendCode(tab, $"<br/>{p.TypeDB} - {p.NameDB}", 1);
                var complement = p.LengthMain.HasValue ? $"({p.LengthMain}$$$)" : "";
                complement = complement.Replace("$$$", p.LengthDecimal.HasValue ? $", {p.LengthDecimal}" : "");

                entityList.AppendLine($"|{p.NameDB}|{p.TypeDB}{complement}|{notNull}|{key}|{auto}|{parent}||");
            }
        }
